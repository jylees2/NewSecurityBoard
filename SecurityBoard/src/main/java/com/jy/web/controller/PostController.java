...
import com.jy.config.auth.CustomUserDetails;
...
//		model.addAttribute("prev", pageable.previousOrFirst().getPageNumber()); // 이전 페이지 번호
	...
	public String write(@AuthenticationPrincipal UserAdapter user, Model model) {
	...
	public String read(@PathVariable Long id, Model model, @AuthenticationPrincipal UserAdapter user){
		...
		if(user != null) {
			...
			/* 게시물 작성자 본인 확인 */
			if( post.getUserId().equals(user.getUserDto().getId()) ) {
	...
	public String update(@PathVariable Long id, Model model, @AuthenticationPrincipal UserAdapter user) {
	...
	public String search(@RequestParam String keyword, Model model,
				@RequestParam(required = false, defaultValue = "0", value="page") int pageNo,
				Pageable pageable, @AuthenticationPrincipal UserAdapter user) {
		pageNo = (pageNo == 0) ? 0 : (pageNo - 1);
